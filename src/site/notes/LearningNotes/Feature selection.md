...
examples:
- [**Lasso / L1 regularization**]([[Regression#Lasso Regression]]): can shrink some coefficients to exactly zero, so it performs feature selection
- [**Ridge / L2 regularization**]([[Regression#Ridge Regression]]): shrinks coefficients but usually does not set them to exactly zero, so it is not direct feature selection
