...
- it can be also seen as a special case of [[LearningNotes/Intelligent Agent\|Intelligent Agent]] where the retriever is a tool the model can use
### Why RAG is needed
- LLM faces challenges
	- domain knowledge
	- hallucinations
	- training date cut off (outdated training set)
- These challenges maybe can be tackled by [[LearningNotes/LLM Finetuning\|LLM Finetuning]], but it's expensive to train the LLM
> [! llm Fine-tuning]
