...
3. train model: training, [[Error analysis\|error analysis]] & iterative improvement -> loop between 2 and 3 until your model is done
...
{ #8a1aad}
	...
	- label/prior drift: data distribution of target variables shifts (the output distribution changes but, for a given output, the input distribution stays the same)
	- concept/posterior drift: the definition of label itself changes based on a particular feature (the input distribution remains the same but the conditional distribution of the output given an input changes)
...
## Useful techniques in some contexts
...
- How to evaluate model's performance before it's deployed: [[Notes/Model Offline Evaluation\|Model Offline Evaluation]]
