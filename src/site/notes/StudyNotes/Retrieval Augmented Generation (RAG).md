---
{"topic":"LargeLanguageModel, AIEngineering","dg-publish":true,"permalink":"/StudyNotes/Retrieval Augmented Generation (RAG)/","dgPassFrontmatter":true,"noteIcon":""}
---
...
- it can be also seen as a special case of [[StudyNotes/Intelligent Agent\|Intelligent Agent]] where the retriever is a tool the model can use
### Why RAG is needed
- LLM faces challenges
	- domain knowledge
	- hallucinations
	- training date cut off (outdated training set)
- These challenges maybe can be tackled by [[StudyNotes/LLM Finetuning\|LLM Finetuning]], but it's expensive to train the LLM
> [! llm Fine-tuning]
